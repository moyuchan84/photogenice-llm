    ...
    *,
    ...
    for equipment_id, group in _group_by_equipment(rows).items():
        ...
        for session in _split_into_sessions(ordered, session_gap_sec):
            chunks.extend(
                _chunk_session(
                    equipment_id, session, error_window_before, error_window_after, error_levels
                )
            )
    return chunks
...
def _chunk_session(
    ...
    if not error_indices:
        return [_build_chunk(equipment_id, session, error_codes=[])]
    ...
    for start, end in merged:
        ...
        result.append(_build_chunk(equipment_id, window_rows, error_codes=error_codes))
    return result


def _build_chunk(equipment_id: str, rows: list[RawLogRow], error_codes: list[str]) -> LogChunk:
